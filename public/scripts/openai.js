...
async function prepareOpenAIMessages({
    ...
    if (oai_settings.jailbreak_system && jailbreak) {
        const jailbreakMessage = {
            "role": "system",
            "content": substituteParams(jailbreak, name1, name2, oai_settings.jailbreak_prompt)
        };
    ...
    if (type == 'continue') {
        const continueNudge = {
            "role": "system",
            "content": stringFormat('[Continue the following message. Do not include ANY parts of the original message. Use capitalization and punctuation as if your reply is a part of the original message:\n\n{0}]', cyclePrompt || '')
        };
...
async function sendOpenAIRequest(type, openai_msgs_tosend, signal) {
    ...
    if (extension_settings.slaudeChu.slaudeChuAuto_connect && generate_data.model === 'slack-claude') {
        generate_data.slack = {
            TOKEN: extension_settings.slaudeChu.TOKEN,
            COOKIE: extension_settings.slaudeChu.COOKIE,
            TEAM_ID: extension_settings.slaudeChu.TEAM_ID,
            CHANNEL: extension_settings.slaudeChu.CHANNEL,
            CLAUDE_USER: extension_settings.slaudeChu.CLAUDE_USER,
            MAINPROMPT_LAST: extension_settings.slaudeChu.MAINPROMPT_LAST,
            MAINPROMPT_AS_PING: extension_settings.slaudeChu.MAINPROMPT_AS_PING,
            USE_BLOCKS: extension_settings.slaudeChu.USE_BLOCKS,
            STREAMING_TIMEOUT: extension_settings.slaudeChu.STREAMING_TIMEOUT,
            PING_MESSAGE: extension_settings.slaudeChu.PING_MESSAGE
        };
    }
...
async function getStatusOpen() {
    if (is_get_status_openai) {
        ...
        let data = {
            ...
            use_chu_set_model: oai_settings.openai_model,
        };
...
function getMaxContextOpenAI(value) {
    if (oai_settings.max_context_unlocked) {
        return unlocked_max;
    } else if (['claude-2.0'].includes(value)) {
        return claude_100k_max;
    } else if (['gpt-4', 'gpt-4-0314', 'gpt-4-0613', 'slack-claude'].includes(value)) {
        return max_8k;
