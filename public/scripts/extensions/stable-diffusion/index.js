...
async function loadSettings() {
    ...
    $('#sd_API_set_vl').val(extension_settings.sd.sd_API_set_vl).trigger('input');
    $('#sd_API_set').prop('checked', extension_settings.sd.sd_API_set);
...
async function onsetAPIInput() {
    extension_settings.sd.sd_API_set = !!$(this).prop('checked');
    saveSettingsDebounced();
}
...
function onsetAPIVLInput() {
    extension_settings.sd.sd_API_set_vl = $('#sd_API_set_vl').val();
    resetScrollHeight($(this));
    saveSettingsDebounced();
}
...
async function generatePicture(_, trigger, message, callback) {
    ...
    if (!modules.includes('sd') && (!extension_settings.sd.horde && !extension_settings.sd.sd_API_set)) {
...
async function generatePrompt(quiet_prompt) {
    ...
    if (extension_settings.sd.refine_mode) {
        const refinedPrompt = await callPopup('<h3>Review and edit the generated prompt:</h3>Press "Cancel" to abort the image generation.', 'input', reply, {
            rows: 5,
            okButton: 'Generate'
        });
...
async function generateExtrasImage(prompt, callback) {
    ...
    console.log(extension_settings.sd.sd_API_set_vl);
    if (extension_settings.sd.sd_API_set) {
    ...
    if (result.ok) {
        ...
        console.log(data);
        if (extension_settings.sd.sd_API_set) {
            base64Image = `data:image/jpeg;base64,${data.images[0]}`;
        } else {
...
jQuery(async () => {
    ...
    <div class="sd_settings">
        ...
        <div class="inline-drawer-content">
            ...
            <label for="sd_API_set" class="checkbox_label" title="自定义SD接口地址">
                <input id="sd_API_set" type="checkbox" />
                自定义SD接口
            </label>
            <textarea id="sd_API_set_vl" class="text_pole textarea_compact" rows="2" placeholder="输入SD接口url地址，如本地SD默认地址 http://localhost:7860"></textarea>
            </div>
    ...
    $('#sd_API_set').on('input', onsetAPIInput);
    $('#sd_API_set_vl').on('input', onsetAPIVLInput);
