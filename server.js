...
function splitPrompt(text, prompts, maxMessageLength) {
    ...
    if (secondHalf > maxMessageLength) {
        return splitPrompt(secondHalf, prompts, maxMessageLength);
    }
    return secondHalf;
}
...
function buildSlackPromptMessages(messages, config, maxMessageLength) {
    let prompts = [];
    let currentPrompt = '';
    let mainPrompt = '';
    if (config.MAINPROMPT_LAST || config.MAINPROMPT_AS_PING) {
        ...
        if (index > 0) {
            ...
            if (currentPrompt.length > maxMessageLength) {
                currentPrompt = splitPrompt(currentPrompt, prompts, maxMessageLength);
            }
            messages.shift();
        } else {
...
async function sendslackClaudeRequest(req, res) {
    const config = req.body.slack
    ...
    try {
        ...
        let promptMessages = buildSlackPromptMessages(req.body.messages, config, maxMessageLength);

        let tsThread = await createSlackThread(promptMessages[0], config);
        ...
        if (promptMessages.length > 1) {
            for (let i = 1; i < promptMessages.length; i++) {
                await createSlackReply(promptMessages[i], tsThread, config);
                console.log(`Created ${i}. reply on thread ${tsThread}`);
            }
        }

        let ws = await openWebSocketConnection(config);
        let timeout = null;
        ...
        await createClaudePing(pingMessage, tsThread, config);
...
function preparePingMessage(msg, config) {
...
async function postSlackMessage(msg, thread_ts, pingClaude, config) {
    ...
    if (pingClaude) {
        msg = preparePingMessage(msg, config);
    }
...
async function createSlackThread(promptMsg, config) {
    return await postSlackMessage(promptMsg, null, false, config);
}

async function createSlackReply(promptMsg, ts, config) {
    return await postSlackMessage(promptMsg, ts, false, config);
}

async function createClaudePing(promptMsg, ts, config) {
    return await postSlackMessage(promptMsg, ts, true, config);
}
